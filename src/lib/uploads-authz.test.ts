...
describe("canViewDocument (mirrors /api/documents/[id])", () => {
  ...
  it("owner can view their own user-owned document", () => {
    expect(canViewDocument(userOwned, { userId: "user_abc", role: "HEALER" })).toBe(true);
  });

  it("a different healer cannot view someone else's user-owned document", () => {
    expect(canViewDocument(userOwned, { userId: "user_def", role: "HEALER" })).toBe(false);
  });

  it("ADMIN can view any user-owned document", () => {
    expect(canViewDocument(userOwned, { userId: "admin_1", role: "ADMIN" })).toBe(true);
  });

  it("SUPER_ADMIN can view any user-owned document", () => {
    expect(canViewDocument(userOwned, { userId: "sa_1", role: "SUPER_ADMIN" })).toBe(true);
  });

  it("QUALITY_CONTROLLER can view any user-owned document", () => {
    expect(canViewDocument(userOwned, { userId: "qc_1", role: "QUALITY_CONTROLLER" })).toBe(true);
  });

  it("COORDINATOR cannot view a user-owned document they don't own", () => {
    expect(canViewDocument(userOwned, { userId: "coord_1", role: "COORDINATOR" })).toBe(false);
  });

  it("COUNSELLOR cannot view a user-owned document they don't own", () => {
    expect(canViewDocument(userOwned, { userId: "couns_1", role: "COUNSELLOR" })).toBe(false);
  });
  ...
  it("client-owned documents have null ownerUserId — non-elevated user fails ownership check", () => {
    ...
    expect(canViewDocument(clientOwned, { userId: "any_user", role: "HEALER" })).toBe(false);
  });

  it("client-owned documents are visible to elevated roles", () => {
    expect(canViewDocument(clientOwned, { userId: "qc_1", role: "QUALITY_CONTROLLER" })).toBe(true);
  });
  ...
  it("ownerUserId being empty string does not accidentally match", () => {
    ...
    // regression check.)
    expect(canViewDocument(clientOwned, { userId: "", role: "HEALER" })).toBe(false);
  });
});
