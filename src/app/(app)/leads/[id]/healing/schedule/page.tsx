...
import { scheduleHealingSessionAction } from "./actions";

export const metadata = { title: "Schedule healing session" };

export default async function ScheduleHealingPage({
  params,
  searchParams,
}: {
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6">
      ...
      <header>
        <h1 className="font-serif text-3xl font-medium tracking-tight">Schedule healing session</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          WhatsApp confirmations will be sent to the client and healer automatically.
        </p>
      </header>
      <form
        action={scheduleHealingSessionAction}
      ...
      >
        ...
        <Field id="mode" label="Mode" required>
          <select id="mode" name="mode" defaultValue="IN_PERSON" className={inputCls}>
            <option value="IN_PERSON">In-person (centre)</option>
            <option value="DISTANT">Distant (online)</option>
          </select>
        </Field>

        <Field id="healerId" label="Assigned healer" required>
          <select
            id="healerId"
            name="healerId"
