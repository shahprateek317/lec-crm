    ...
    <div className="space-y-6">
      ...
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Prepaid healing packages. Each credit = one healing session. Post-credit rate is ₹500 per session.
        </p>
      </header>

      {sp.ok && (
        <p className="rounded-md bg-emerald-100 px-3 py-2 text-sm text-emerald-900">Saved.</p>
      )}
      {sp.error && (
      ...
      <Card className="rounded-xl">
        <CardContent className="p-6">
          ...
          <form action={createPackageAction} className="grid gap-3 md:grid-cols-[2fr_1fr_1fr_auto]">
            ...
            <input name="amount" type="number" min={1} placeholder="Amount (₹)" required className={inputCls} />
